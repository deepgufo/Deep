...
export default function SceltaPersonaggio() {
  ...
  useEffect(() => {
    async function loadData() {
      // MODIFICA: Rimosso il reindirizzamento forzato se non c'è user
      const { data: { user } } = await supabase.auth.getUser();
      
      let gender = 'attore'; // Genere di default per gli ospiti
      ...
      <div className="w-full max-w-md bg-black h-full flex flex-col relative shadow-2xl shadow-yellow-900/20 border-x border-white/5">
        ...
        <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-[60] w-max animate-bounce">
          <div className="bg-black/60 backdrop-blur-lg border border-yellow-500/60 px-4 py-2 rounded-full shadow-[0_0_20px_rgba(234,179,8,0.2)] flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-white uppercase tracking-[0.15em] whitespace-nowrap">
