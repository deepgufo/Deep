...
export default function UtentiPage() {
  ...
  const fetchUtentiData = async () => {
    try {
      ...
      // IMPORTANTE: Leggiamo tutti i video per calcolare la distribuzione reale
      ...
      if (profiles) {
        ...
        const returningCount = profiles.filter(p => p.created_at < startOfToday && p.last_seen && p.last_seen > startOfToday).length;
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#0A0A0A] text-white font-sans overflow-hidden flex flex-col">
      ...
      <div className="flex-1 flex flex-col p-6 overflow-y-auto custom-scrollbar bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-zinc-900/20 via-black to-black">
        ...
        <div className="w-full max-w-7xl mx-auto mb-10">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="bg-zinc-900/40 border border-white/5 p-6 rounded-[32px] backdrop-blur-md relative overflow-hidden group hover:bg-zinc-900/60 transition-colors border-l-4 border-l-yellow-400">
              ...
              <p className="text-[8px] text-zinc-500 font-bold uppercase mt-2">Iscritti ieri, tornati oggi</p>
            </div>
            <div className="bg-zinc-900/40 border border-white/5 p-6 rounded-[32px] backdrop-blur-md relative overflow-hidden group hover:bg-zinc-900/60 transition-colors">
        ...
        <div className="w-full max-w-7xl mx-auto mt-4 mb-10">
          <div className="flex items-center justify-between mb-4 px-2">
            ...
            <div className="bg-red-500/10 border border-red-500/20 px-6 py-3 rounded-2xl text-right">
              <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-1">Signup Drop Rate</p>
              <p className="text-3xl font-black text-red-500 italic">{funnelStats.auth > 0 ? Math.round(((funnelStats.auth - totalUsers) / funnelStats.auth) * 100) : 0}%</p>
            </div>
          </div>
          <div className="overflow-x-auto custom-scrollbar pb-4">
            <div className="min-w-[900px] flex items-center justify-between bg-zinc-900/40 border border-white/5 p-8 rounded-[40px] backdrop-blur-2xl shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col items-center px-4 relative z-10">
                <span className="text-[10px] font-black text-red-500 bg-red-500/10 border border-red-500/20 px-3 py-1 rounded-full mb-3">-{calcDrop(funnelStats.auth, funnelStats.start)}%</span>
              ...
              <div className="flex flex-col items-center px-4 relative z-10">
                <span className="text-[10px] font-black text-red-500 bg-red-500/10 border border-red-500/20 px-3 py-1 rounded-full mb-3">-{calcDrop(funnelStats.start, funnelStats.photo)}%</span>
                <ArrowRight className="w-6 h-6 text-zinc-700" />
              </div>
              <div className="flex flex-col items-center flex-1 relative z-10">
              ...
              <div className="flex flex-col items-center px-4 relative z-10">
                <span className="text-[10px] font-black text-red-500 bg-red-500/10 border border-red-500/20 px-3 py-1 rounded-full mb-3">-{calcDrop(funnelStats.photo, funnelStats.school)}%</span>
                <ArrowRight className="w-6 h-6 text-zinc-700" />
              </div>
              <div className="flex flex-col items-center flex-1 relative z-10">
              ...
              <div className="flex flex-col items-center px-4 relative z-10">
                <span className="text-[10px] font-black text-red-500 bg-red-500/10 border border-red-500/20 px-3 py-1 rounded-full mb-3">-{calcDrop(funnelStats.school, totalUsers)}%</span>
                <ArrowRight className="w-6 h-6 text-zinc-700" />
              </div>
              <div className="flex flex-col items-center flex-1 relative z-10">
                ...
                <p className="text-5xl font-black text-white italic tabular-nums">{totalUsers}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-7xl mx-auto mt-4 mb-10">
          <div className="flex items-center justify-between mb-4 px-2">
            ...
            <div className="bg-yellow-400/10 border border-yellow-400/20 px-6 py-3 rounded-2xl text-right">
          ...
          <div className="overflow-x-auto custom-scrollbar pb-4">
            <div className="min-w-[900px] flex items-center justify-between bg-zinc-900/40 border border-white/5 p-8 rounded-[40px] backdrop-blur-2xl shadow-2xl relative overflow-hidden">
              <div className="flex flex-col items-center flex-1 relative z-10">
                <div className="bg-zinc-500/10 p-4 rounded-2xl mb-4 border border-zinc-500/20"><PlayCircle className="w-6 h-6 text-zinc-400" /></div>
              ...
              <div className="flex flex-col items-center px-6 relative z-10"><ArrowRight className="w-6 h-6 text-zinc-700" /></div>
              <div className="flex flex-col items-center flex-1 relative z-10">
              ...
              <div className="flex flex-col items-center flex-1 relative z-10">
                ...
                <p className="text-5xl font-black text-white italic tabular-nums">{totalSaves}</p>
                <div className="flex gap-4 mt-2">
        ...
        {/* --- STRISCIA 3: DISTRIBUZIONE PRODUZIONE --- */}
        <div className="w-full max-w-7xl mx-auto mt-4 mb-12">
          <div className="flex items-center justify-between mb-6 px-2">
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 px-6 py-3 rounded-2xl text-right">
              ...
              <p className="text-3xl font-black text-blue-400 italic">{totalUsers > 0 ? (totalVideos / totalUsers).toFixed(1) : 0}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-6">
            <div className="bg-zinc-900/40 border border-white/5 p-8 rounded-[40px] backdrop-blur-md relative overflow-hidden group hover:bg-zinc-900/60 transition-colors">
              ...
              <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-zinc-600" style={{ width: `${totalUsers > 0 ? (distribution.zero / totalUsers) * 100 : 0}%` }}></div>
              </div>
              <p className="text-[10px] text-zinc-500 font-bold mt-3">Creato profilo ma zero girato.</p>
            </div>
            <div className="bg-zinc-900/40 border border-white/5 p-8 rounded-[40px] backdrop-blur-md relative overflow-hidden group hover:bg-zinc-900/60 transition-colors">
              ...
              <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500" style={{ width: `${totalUsers > 0 ? (distribution.one / totalUsers) * 100 : 0}%` }}></div>
              </div>
              <p className="text-[10px] text-blue-400/60 font-bold mt-3">Provato una volta.</p>
            </div>
            <div className="bg-yellow-400/5 border border-yellow-400/20 p-8 rounded-[40px] backdrop-blur-md relative overflow-hidden ring-1 ring-yellow-400/20 group hover:bg-yellow-400/10 transition-colors">
              <Trophy className="absolute -bottom-4 -right-4 w-24 h-24 text-yellow-400/10 group-hover:text-yellow-400/20 transition-colors" />
              <h4 className="text-[11px] font-black uppercase tracking-widest text-yellow-400 mb-3">Star (2+ video)</h4>
              <p className="text-6xl font-black italic text-yellow-400 tabular-nums mb-6">{distribution.twoPlus}</p>
              <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-yellow-400" style={{ width: `${totalUsers > 0 ? (distribution.twoPlus / totalUsers) * 100 : 0}%` }}></div>
              </div>
              <p className="text-[10px] text-yellow-400/80 font-bold mt-3">Heavy users.</p>
            </div>
          </div>
        </div>

        {/* --- STRISCIA 4: SCHOOL LEADERBOARD --- */}
        <div className="w-full max-w-7xl mx-auto mt-4 mb-20">
          <div className="flex items-center justify-between mb-6 px-2">
            <div>
              ...
              <p className="text-zinc-500 text-[10px] uppercase tracking-[0.2em] font-medium">Validazione sul territorio</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 p-8 rounded-[40px] relative overflow-hidden backdrop-blur-md">
              ...
              <p className="text-7xl font-black italic text-white tabular-nums">{schoolStats.polo}</p>
            </div>
            <div className="bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/20 p-8 rounded-[40px] relative overflow-hidden backdrop-blur-md">
              ...
              <p className="text-7xl font-black italic text-white tabular-nums">{schoolStats.montale}</p>
            </div>
            <div className="bg-gradient-to-br from-rose-500/10 to-transparent border border-rose-500/20 p-8 rounded-[40px] relative overflow-hidden backdrop-blur-md">
              ...
              <p className="text-7xl font-black italic text-white tabular-nums">{schoolStats.fermi}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <footer className="p-4 bg-black/80 backdrop-blur-md border-t border-white/5 opacity-50 text-center mt-auto flex-shrink-0 z-50 relative">
        <p className="text-[8px] font-bold tracking-[0.5em] uppercase text-zinc-500">Private access only - secure encrypted session</p>
      </footer>
